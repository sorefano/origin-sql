...
Contact AS (
    SELECT
        ...
        Lpar_per_title,
        Lpar_per_initials,
        CAST(Lpar_per_date_of_birth AS STRING) AS Lpar_per_date_of_birth,
        ...
        Lpar_phone,
        Lpar_tcy_ind,
        CAST(Lpar_tin_main_tenant_ind AS STRING) AS Lpar_tin_main_tenant_ind,
        Lpar_tin_stat_successor_ind,
        CAST(Lpar_per_alt_ref AS STRING) AS Lpar_per_alt_ref,
    ...
    SELECT
        ...
        CAST(NULL AS STRING) AS Lpar_tin_main_tenant_ind,
        ...
        CAST(NULL AS STRING) AS Lpar_cos_code_update,
...
--     SELECT
--         T.Ltcy_alt_ref,
--         C.Primary_account_tenancy_id,
--     FROM Tenancy AS T
--     INNER JOIN Contact AS C ON T.Tenancy_id = C.Primary_account_tenancy_id
-- ),
...
--         CASE
--             WHEN CAST(O.Occupancy_end_date AS DATETIME) = CAST("9999-12-31" AS DATE) THEN NULL
--             ELSE CAST(O.Occupancy_end_date AS DATETIME)
--         END
--             AS Lpar_hop_end_date,
--         CASE
--             WHEN CAST(O.Occupancy_end_date AS DATETIME) < CAST("9999-12-31" AS DATE) THEN "DATALOAD"
--         END
--             AS Lpar_hop_hper_code,
...
--     LEFT OUTER JOIN Missing_tenancy AS Mt
--         ON C.Primary_account_tenancy_id = Mt.Primary_account_tenancy_id
--     LEFT OUTER JOIN
--         Tenancy AS T_former
...
--     C.*,
--     T.*,
-- FROM Contact_hist AS C
-- INNER JOIN Title_dob AS T ON C.contact_id = T.contact_id

-- -- FROM Contact_hist_raw C the title miss match does not appear in raw