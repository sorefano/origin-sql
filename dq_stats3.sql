...
FROM gc-t-prj-originmig.Origin_Migration_test_public.__TABLES__ t1
JOIN  gc-t-prj-originmig.Origin_Migration_test_public.__TABLES__ t2
...
SELECT count(*) ,Is_missing_Laba_pay_ref, Is_missing_Laba_balance, Is_missing_Laba_balance_date, Is_Laba_balance_date_future
 FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_account_balances`
...
SELECT count(*) , Is_missing_lcde_legacy_ref, Is_missing_lcde_contact_name, Is_missing_lcde_comments
 FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_contact_details`
...
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_ofv_housing_group`
GROUP BY Is_missing_lpva_char_value	

-- dq_dim_people
SELECT COUNT(*) AS cnt,Is_missing_occupant_type_code,Is_missing_household_start_date	 
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_people`
...
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_physical_au_addresses`
...
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_property`
...
SELECT COUNT(*) AS cnt, Is_missing_postcode, Is_missing_area_or_town
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_property_addresses`
GROUP BY Is_missing_postcode, Is_missing_area_or_town

-- dq_dim_revenue_accounts
SELECT COUNT(*) AS cnt,Is_missing_pay_ref, Is_missing_account_type_code
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_revenue_accounts`
...
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_dim_tenancy`
...
FROM `gc-t-prj-originmig.Origin_Migration_test_public.dq_fct_estates_property_elements`
